...
def _interface_energy_residual_full(T_s_guess: float,
    ...
    try:
        ...
        # ### <<< 追加点: ここからが大きな変更ブロック >>>
        # --- 2. 乱れを考慮したNu数、Sh数の計算 ---
        if k > 1e-9 and not np.isnan(mu_g_s) and not np.isnan(rho_g_s):
            u_rel_turb = np.sqrt(2.0/3.0 * k)
            Re_t = rho_g_s * u_rel_turb * (2*R) / mu_g_s
            Pr = cp_g_s * mu_g_s / lambda_g_s
            Nu = 2.0 + 0.6 * np.sqrt(Re_t) * (Pr**(1.0/3.0))
            Sh = Nu # 簡単のためLe=1と仮定
        else:
            Nu = 2.0 # 乱れがない場合は静止場の理論値
            Sh = 2.0

        # --- 3. 熱伝達率 h と物質伝達率 h_m の計算 ---
        D_fuel_s = Dk_s[fuel_idx] # 燃料の分子拡散係数
        h_conv = Nu * lambda_g_s / (2 * R)
        h_m = Sh * D_fuel_s / (2 * R)

        # --- 4. トランスファ数ベースでの質量流束計算 ---
        B_m = Y_f_eq / (1.0 - Y_f_eq) if Y_f_eq < 0.99999 else np.inf
        mdot_double_prime = h_m * rho_g_s * np.log(1.0 + B_m) if B_m < np.inf else h_m * rho_g_s * 20
        mdot_double_prime = max(0.0, mdot_double_prime)

        # --- 5. トランスファ数ベースでの熱流束計算 ---
        q_gas_conv_in = h_conv * (T_g_node0 - T_s_phys)
        B_h = (cp_g_s * (T_g_node0 - T_s_phys)) / Lv if Lv > 1e-3 else 0.0
        
        correction_factor = 1.0
        if abs(B_h) > 1e-9:
            correction_factor = B_h / (np.exp(B_h) - 1.0)
        
        q_gas_in_total = q_gas_conv_in * correction_factor
        q_gas_out_at_surf = -q_gas_in_total # 向きを定義に合わせる
...
def _residual_wrapper_for_solver(T_s_guess, T_l_node_last, T_g_node0, Y_g_node0, P, R, r_l_node_last_center, r_g_node0_center, gas_props, liquid_props, Nl, fugacity_interpolator, k, epsilon):
    ...
    residual, _, _, _ = _interface_energy_residual_full(T_s_guess, T_l_node_last, T_g_node0, Y_g_node0, P, R, r_l_node_last_center, r_g_node0_center, gas_props, liquid_props, Nl, fugacity_interpolator, k, epsilon)
    return residual
...
def solve_interface_conditions(
    ...
    try:
        ...
        res_low, _, _, _ = _interface_energy_residual_full(T_lower_bound, T_l_node_last, T_g_node0, Y_g_node0, P, R, r_l_node_last_center, r_g_node0_center, gas_props, liquid_props, Nl, fugacity_interpolator, k, epsilon)
        res_high, _, _, _ = _interface_energy_residual_full(T_upper_bound, T_l_node_last, T_g_node0, Y_g_node0, P, R, r_l_node_last_center, r_g_node0_center, gas_props, liquid_props, Nl, fugacity_interpolator, k, epsilon)
