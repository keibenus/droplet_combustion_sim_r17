...
class GasProperties:
    ...
    def get_effective_transport_properties(self, T, P, Y, k, epsilon):
        """
        ...
        if k > 1e-9 and epsilon > 1e-9 and rho > 1e-9:
            # 乱流粘性係数 mu_t = C_mu * rho * k^2 / epsilon
            mu_t = config.CMU * rho * k**2 / epsilon
            
            # 乱流熱伝導率 lambda_t = mu_t * cp / Pr_t
            lambda_t = mu_t * cp / config.TURBULENT_PRANDTL_NUMBER
            
            # 乱流拡散係数 D_t = mu_t / (rho * Sc_t)
            D_t = mu_t / (rho * config.TURBULENT_SCHMIDT_NUMBER)
        else:
            lambda_t = 0.0
            D_t = 0.0
